...
#define RTREE_QUAL RTree_EEP<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>
...
//=========================================================
  Rect* getRect(Node* pNode, int count)
  {
	  count = pNode->m_count;
	  Rect* tempRect = new Rect[count];
	  for(int i = 0; i < count; i++)
		  tempRect[i] = pNode->m_branch[i].m_rect;
	  return tempRect;
  }

  Point* SearchNN(Point& p, float *quadMin, float *quadMax, Node* pNode)
  {
	  Point *tempPoint, pPoint;
	  MinHeap<Branch> H(HEAP_MAX_SIZE);
	  tempPoint = SearchNN(p, quadMin, quadMax,pNode, H);
	  if(tempPoint == NULL)
		  return NULL;
  ...
  //search NN for point p
  Point* SearchNN(Point p, float *quadMin, float *quadMax, Node* pNode, MinHeap<Branch>& H)
  {
	  ...
	  pPoint = NULL;
	  Rect MBR;
	  MBR.m_min[0] = quadMin[0];
	  MBR.m_min[1] = quadMin[1];
	  MBR.m_max[0] = quadMax[0];
	  MBR.m_max[1] = quadMax[1];
	  DistCompute help;

	  if(H.Top(b) && pNode == NULL)
	  {
		  temp.x = b->m_rect.m_min[0];
		  temp.y = b->m_rect.m_min[1];
		  return &temp;
	  }
	  if(pNode->IsLeaf())
	  {
		//  if(Overlap(&MBR, &b->m_rect))
		  {
			  H.DeleteTop();
			  for(int index = 0; index < pNode->m_count; index++)
			  {
				  dist = help.minDist(p, pNode->m_branch[index].m_rect);
				  pNode->m_branch[index].min_dist = dist;
				  pNode->m_branch[index].m_child = NULL;
				  H.Insert(pNode->m_branch[index]);
			  }
			  
		  }
	  }
	  //t is an internode, insert all its children into heap
	  else if(pNode->IsInternalNode())
	  {
		  H.DeleteTop();
		  for(int index = 0; index < pNode->m_count; index++)
		  {
	  ...
	  while(H.Top(b))
	  {
		  if(Overlap(&MBR, &b->m_rect))
		  {
			  pPoint = SearchNN(p, MBR.m_min, MBR.m_max, b->m_child, H);
			  if(pPoint == NULL)
				  return NULL;
			  temp.x = pPoint->x;
			  temp.y = pPoint->y;
...
//=========================================================
  ...
  private:
    ...
    friend RTree_EEP; // Allow hiding of non-public functions while allowing manipulation by logical owner
  };
...
public:
  ...
  bool TotalArea(Node* pNode, float**total);
  //================================================================
  Node* AllocNode();
  ...
  bool Search(Node* a_node, Rect* a_rect, ListPoint **searchResult, int& a_foundCount, bool __cdecl a_resultCallback(DATATYPE a_data, void* a_context), void* a_context);
...
RTREE_TEMPLATE
typename Node* RTREE_QUAL::AllocNode()
{
  Node* newNode;
#ifdef RTREE_DONT_USE_MEMPOOLS
  newNode = new Node;
...
RTREE_TEMPLATE
typename ListNode* RTREE_QUAL::AllocListNode()
{
#ifdef RTREE_DONT_USE_MEMPOOLS
  return new ListNode;
#else // RTREE_DONT_USE_MEMPOOLS
...
RTREE_TEMPLATE
typename Rect RTREE_QUAL::NodeCover(Node* a_node)
{
  ASSERT(a_node);
...
RTREE_TEMPLATE
typename Rect RTREE_QUAL::CombineRect(Rect* a_rectA, Rect* a_rectB)
{
...
RTREE_TEMPLATE
int RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], ListPoint **searchResultArrary, bool __cdecl a_resultCallback(DATATYPE a_data, void* a_context), void* a_context)
{
#ifdef _DEBUG
	for(int index=0; index<NUMDIMS; ++index)
	{
...
//========================================================= searchResult is added by Zeyi Wen ===========================
RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, ListPoint **searchResult, int& nodeCount, bool __cdecl a_resultCallback(DATATYPE a_data, void* a_context), void* a_context)
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
...
RTREE_TEMPLATE
bool RTREE_QUAL::TotalArea(Node* pNode, float **total)
{
	Rect r;
	r = pNode->m_branch[0].m_rect;
	for(int index = 1; index < pNode->m_count; index++)
	{
		r = CombineRect(&pNode->m_branch[index].m_rect, &r);
	}
	(*total)[0] = r.m_min[0];
	(*total)[1] = r.m_min[1];
	(*total)[2] = r.m_max[0];
