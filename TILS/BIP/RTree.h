...
public:
  ...
  int SooneSearch(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS]);
  int SooneSearchNoNLC(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS]);
  int SooneSearchOverlap(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS],
	  Rect* overlap, DATATYPE * tag, int& a_recordCount);
  ...
  // need to access root
  // --------------------------------
  Node * SooneGetRoot(){return m_root;};
  int SooneAggregateCount(const Rect rect, Node* node, int &access)
  {
	  
	  int tempSum = 0;
	  for(int b = 0; b < node->m_count; b++)
	  {
		  
		  if(rect.m_min[0] <= node->m_branch[b].m_rect.m_min[0]
		  && rect.m_min[1] <= node->m_branch[b].m_rect.m_min[1]
		  && rect.m_max[0] >= node->m_branch[b].m_rect.m_max[0]
		  && rect.m_max[1] >= node->m_branch[b].m_rect.m_max[1])
		  {
			  access++;
			  if(node->IsLeaf())
			  {
				  //tempSum++;
				  tempSum += node->aggregateCount;
			  }
			  else
			  {
				  tempSum += node->m_branch[b].m_child->aggregateCount;
			  }
		  }
		  else
		  {
			  Rect overlapRect;
			  overlapRect.m_min[0] = max(rect.m_min[0], node->m_branch[b].m_rect.m_min[0]);
			  overlapRect.m_min[1] = max(rect.m_min[1], node->m_branch[b].m_rect.m_min[1]);
			  overlapRect.m_max[0] = min(rect.m_max[0], node->m_branch[b].m_rect.m_max[0]);
			  overlapRect.m_max[1] = min(rect.m_max[1], node->m_branch[b].m_rect.m_max[1]);
			  if(overlapRect.m_min[0] <= overlapRect.m_max[0]
			  && overlapRect.m_min[1] <= overlapRect.m_max[1])
			  {
				  tempSum += SooneAggregateCount(rect, node->m_branch[b].m_child, access);
			  }
		  }
	  }
	  return tempSum;
...
//protected:
 public:
  ...
  bool SooneSearch(Node* a_node, Rect* a_rect, int& a_foundCount);
  bool SooneSearchNoNLC(Node* a_node, Rect* a_rect, int& a_foundCount);
  bool SooneSearchOverlap(Node* a_node, Rect* a_rect, int& a_foundCount, Rect * overlap, DATATYPE *tag, int& a_recordCount);
  void RemoveAllRec(Node* a_node);
  void Reset();
  void CountRec(Node* a_node, int& a_count);
  void SooneCountRec(Node * a_node, int& a_count, int& l_count);
...
RTREE_TEMPLATE
int RTREE_QUAL::SooneSearch(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS])
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
  ...
  int foundCount = 0;
  SooneSearch(m_root, &rect, foundCount);

  return foundCount;
}

RTREE_TEMPLATE
int RTREE_QUAL::SooneSearchNoNLC(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS])
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
	  if(!(a_min[index] <= a_max[index]))
	  {
		  int here = 0;
	  }
    ASSERT(a_min[index] <= a_max[index]);
  ...
  int foundCount = 0;
  SooneSearchNoNLC(m_root, &rect, foundCount);

  return foundCount;
}

RTREE_TEMPLATE
int RTREE_QUAL::SooneSearchOverlap(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS],
									Rect * overlap, DATATYPE * tag, int& a_recordCount)
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  ...
  // NOTE: May want to return search result another way, perhaps returning the number of found elements here.
	int foundCount = 0;
  SooneSearchOverlap(m_root, &rect, foundCount ,overlap, tag, a_recordCount);
  return foundCount;
}
...
RTREE_TEMPLATE
int RTREE_QUAL::SooneCount(int &countLeaf)
{
	int count = 0;
	countLeaf = 0;
	SooneCountRec(m_root, count, countLeaf);
...
void RTREE_QUAL::CountRec(Node* a_node, int& a_count)
{
  if(a_node->IsInternalNode())  // not a leaf node
  {
    for(int index = 0; index < a_node->m_count; ++index)
    {
	  ...
	  a_node->aggregateCount += a_node->m_branch[index].m_child->aggregateCount;
    }
  }
  else // A leaf node
  {
    a_count += a_node->m_count;
    //a_count++;
	a_node->aggregateCount = a_node->m_count;
  }
}

RTREE_TEMPLATE
void RTREE_QUAL::SooneCountRec(Node* a_node, int& a_count, int& l_count)
{
  if(a_node->IsInternalNode())  // not a leaf node
  {
	a_count++;
    for(int index = 0; index < a_node->m_count; ++index)
    {
      SooneCountRec(a_node->m_branch[index].m_child, a_count, l_count);
	  //a_node->aggregateCount += a_node->m_branch[index].m_child->aggregateCount;
    }
  }
  else // A leaf node
...
// Search in an index tree or subtree for all data retangles that overlap the argument rectangle.
RTREE_TEMPLATE
bool RTREE_QUAL::SooneSearch(Node* a_node, Rect* a_rect, int& a_foundCount)
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  ...
  if(a_node->IsInternalNode()) // This is an internal node in the tree
  {
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        if(!SooneSearch(a_node->m_branch[index].m_child, a_rect, a_foundCount))
        {
          return false; // Don't continue searching
        }
      }
  ...
  else // This is a leaf node
  {
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
		  //may not overlap with the NLC
		  ELEMTYPE center [DIMENSION];
		  ELEMTYPE point [DIMENSION];
		  ELEMTYPEREAL theDist = 0.0 ;
		  ELEMTYPE minDist = 0.0;
		  for(int i = 0; i <DIMENSION; ++i)
		  {
			  center[i] = (a_node->m_branch[index].m_rect.m_min[i] + a_node->m_branch[index].m_rect.m_max[i]) / 2;
			  point[i] = (a_rect->m_min[i] + a_rect->m_max[i]) / 2;
			  theDist += pow((center[i] - point[i]), DIMENSION);
			  minDist = fabs((a_node->m_branch[index].m_rect.m_min[i] - a_node->m_branch[index].m_rect.m_max[i])) / 2;
			  minDist = minDist / pow(2.0, 0.5);
		  }
		  theDist = pow(theDist, (1.0 / (double)DIMENSION));
		  if(theDist < minDist || fabs(theDist - minDist) < EPSILON)
		  {
			  ++a_foundCount;
		  }
...
RTREE_TEMPLATE
bool RTREE_QUAL::SooneSearchNoNLC(Node* a_node, Rect* a_rect, int& a_foundCount)
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  ...
  if(a_node->IsInternalNode()) // This is an internal node in the tree
  {
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        if(!SooneSearchNoNLC(a_node->m_branch[index].m_child, a_rect, a_foundCount))
        {
          return false; // Don't continue searching
        }
      }
...
RTREE_TEMPLATE
bool RTREE_QUAL::SooneSearchOverlap(Node* a_node, Rect* a_rect, int& a_foundCount,Rect * overlap, DATATYPE * tag, int& a_recordCount)
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  ...
  if(a_node->IsInternalNode()) // This is an internal node in the tree
  {
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        if(!SooneSearchOverlap(a_node->m_branch[index].m_child, a_rect,a_foundCount, overlap, tag, a_recordCount))
        {
          return false; // Don't continue searching
        }
      }
  ...
  else // This is a leaf node
  {
    for(int index=0; index < a_node->m_count; ++index)
    {
		bool countAlready = false;
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
		  overlap[a_foundCount] = (a_node->m_branch[index].m_rect);
		  tag[a_foundCount] = (a_node->m_branch[index].m_data);
		  //overlap++;
		  //tag++;
		  a_foundCount++;
		  if(!countAlready)
